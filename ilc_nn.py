...
# --- DEADZONE / BACKLASH PARAM ---
DEADZONE_RAD = 0.03  # ~ 1.7 derajat; bisa kamu adjust 0.02–0.05

# --- KECEPATAN ---
SPEED_LEVELS = [20, 50, 100]
...
def apply_deadzone_disturbance(prev_theta, command_theta):
    """
    Simulasi deadzone/backlash:
    - prev_theta: sudut aktual sebelumnya [θ1_prev, θ2_prev]
    - command_theta: sudut perintah (ideal + koreksi NN)
    - Kalau |command - prev| < DEADZONE_RAD → dianggap tidak gerak (delta = 0)
      Kalau lebih besar → dikurangi sedikit (seolah ada celah mekanik)
    """
    # hitung delta gerakan yang diperintahkan
    d1 = float(command_theta[0] - prev_theta[0])
    d2 = float(command_theta[1] - prev_theta[1])
    
    def deadzone(delta, dz=DEADZONE_RAD):
        if abs(delta) < dz:
            # gerakan terlalu kecil, nyangkut di backlash
            return 0.0
        # di luar deadzone, seolah-olah sebagian gerak hilang
        return delta - dz * math.copysign(1.0, delta)

    d1_eff = deadzone(d1)
    d2_eff = deadzone(d2)

    act_t1 = float(prev_theta[0]) + d1_eff
    act_t2 = float(prev_theta[1]) + d2_eff
    ...
    while not rl.window_should_close():
        ...
        for _ in range(speed_multiplier):
            ...
            if target_cube_idx != -1 and path_idx < len(path):
                ...
                # input NN tetap sama: [θ_curr, θ_ideal]
                ...
                # >>> DI SINI BERUBAH <<<
                # dulu: actual_theta = apply_gravity_disturbance(command_theta[0], command_theta[1])
                actual_theta = apply_deadzone_disturbance(curr_theta, command_theta)
    
                # error yang dipelajari NN tetap: ideal - actual
                error = ideal_theta - actual_theta
                cube_data_X.append(np.concatenate([curr_theta, ideal_theta]))
                cube_data_Y.append(error)
    
                # update sudut sekarang ke sudut aktual (setelah deadzone)
